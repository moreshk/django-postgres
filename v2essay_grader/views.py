...
from .v2grader import check_relevance, hello_world, check_audience_persuasive_grade3
...
def grade_essay_audience(request):
    if request.method == "POST":
        ...
        print(user_response, title, description, essay_type, grade)
        feedback_from_api = check_audience_persuasive_grade3(user_response, title, description, essay_type, grade)
        return JsonResponse({'feedback': feedback_from_api})
